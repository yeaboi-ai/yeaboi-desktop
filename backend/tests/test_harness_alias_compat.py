...
async def test_harness_preview_still_returns_files_shape(client, auth_headers):
    ...
    resp = await client.get(
        f"/api/sessions/{project['id']}/harness/preview", headers=auth_headers
    )
...
async def test_harness_generate_returns_status_shape(client, auth_headers, monkeypatch):
    ...
    resp = await client.post(
        f"/api/sessions/{project['id']}/harness/generate",
        json={"create_repo": False},
        headers=auth_headers,
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    # HarnessStatusResponse shape: id, session_id, status, repo_url, repo_name, created_at
    assert {"id", "session_id", "status", "repo_url", "repo_name", "created_at"} <= set(body.keys())
    assert body["session_id"] == project["id"]
    assert body["status"] == "complete"  # mapped from session_outputs 'ready'
...
async def test_harness_status_after_generate(client, auth_headers, monkeypatch):
    ...
    resp = await client.get(
        f"/api/sessions/{project['id']}/harness/status", headers=auth_headers
    )
    assert resp.status_code == 200
