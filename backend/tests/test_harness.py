...
async def _create_project(client, auth_headers, name: str = "Test Project") -> dict:
...
async def test_harness_preview_returns_file_tree(client, auth_headers):
    ...
    resp = await client.get(f"/api/projects/{project_id}/harness/preview", headers=auth_headers)
...
async def test_harness_preview_with_blueprint_content(client, auth_headers):
    ...
    project = await _create_project(client, auth_headers, name="Blueprint Project")
    project_id = project["id"]
    ...
    assert "Blueprint Project" in files["AGENTS.md"]
...
async def test_harness_generate_creates_config_record(client, auth_headers):
    ...
    resp = await client.post(
        f"/api/projects/{project_id}/harness/generate",
...
async def test_harness_status_returns_config(client, auth_headers):
    ...
    resp = await client.get(f"/api/projects/{project_id}/harness/status", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert data["project_id"] == project_id
    assert data["status"] == "complete"
...
async def test_harness_preview_empty_blueprint_graceful(client, auth_headers):
    ...
    project = await _create_project(client, auth_headers, name="Empty Blueprint Project")
    project_id = project["id"]
