...
async def test_create_with_references_round_trips(client, auth_headers):
    ...
    got = (await client.get(f"/api/projects/{body['id']}", headers=auth_headers)).json()
    assert got["references"] == body["references"]
    assert got["attachments"] == []

    listed = (await client.get("/api/projects", headers=auth_headers)).json()
...
async def test_a_project_without_references_has_an_empty_list(client, auth_headers):
    resp = await client.post("/api/projects", json={"name": "Plain"}, headers=auth_headers)
    assert resp.json()["references"] == []


async def test_patch_replaces_and_dedupes(client, auth_headers):
    created = (await client.post("/api/projects", json={"name": "P", "references": [REF]}, headers=auth_headers)).json()
    ...
    resp = await client.patch(f"/api/projects/{created['id']}", json={"references": []}, headers=auth_headers)
    assert resp.json()["references"] == []

    # A PATCH that says nothing about references leaves them alone.
    await client.patch(f"/api/projects/{created['id']}", json={"references": [REF]}, headers=auth_headers)
    resp = await client.patch(f"/api/projects/{created['id']}", json={"name": "Renamed"}, headers=auth_headers)
...
async def test_malformed_references_are_422(client, auth_headers, bad):
    resp = await client.post("/api/projects", json={"name": "Bad", "references": bad}, headers=auth_headers)
