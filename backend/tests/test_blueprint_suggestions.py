...
async def test_agent_post_creates_pending_suggestion(client, auth_headers):
    ...
    listed = await client.get(
        f"/api/projects/{project_id}/blueprint-suggestions",
        headers=auth_headers,
    )
    ...
    bp = await client.get(f"/api/projects/{project_id}/blueprint", headers=auth_headers)
    assert (bp.json()["content"].get("tech_stack") or "") == ""


async def test_agent_post_rejects_invalid_section(client, auth_headers):
    _, session_id = await _create_session(client, auth_headers)
    resp = await client.post(
        f"/api/internal/sessions/{session_id}/blueprint-suggestions",
...
async def test_accept_merges_into_blueprint(client, auth_headers):
    ...
    resp = await client.post(
        f"/api/projects/{project_id}/blueprint-suggestions/{sid}/accept",
        json={},
        headers=auth_headers,
    )
...
async def test_reject_does_not_touch_blueprint(client, auth_headers):
    ...
    resp = await client.post(
        f"/api/projects/{project_id}/blueprint-suggestions/{sid}/reject",
        headers=auth_headers,
    )
    assert resp.status_code == 200
...
async def test_accept_unknown_id_returns_404(client, auth_headers):
    project_id, _ = await _create_session(client, auth_headers)
    resp = await client.post(
        f"/api/projects/{project_id}/blueprint-suggestions/nonexistent/accept",
        json={},
        headers=auth_headers,
    )
...
async def test_bulk_accept_section_merges_all(client, auth_headers):
    ...
    resp = await client.post(
        f"/api/projects/{project_id}/blueprint-suggestions/bulk-accept",
...
async def test_list_returns_only_pending_status(client, auth_headers):
    ...
    await client.post(
        f"/api/projects/{project_id}/blueprint-suggestions/{sid_a}/reject",
        headers=auth_headers,
    )
...
async def test_list_requires_auth(client):
    resp = await client.get("/api/projects/some-id/blueprint-suggestions")
...
async def test_replace_strips_superseded_bullet(client, auth_headers):
    ...
    await client.patch(
        f"/api/projects/{project_id}/blueprint/sections/team_capacity",
...
async def test_session_blueprint_diff_returns_changed_sections(client, auth_headers):
    ...
    diff = await client.get(
        f"/api/projects/{project_id}/sessions/{session_id}/blueprint-diff",
        headers=auth_headers,
    )
