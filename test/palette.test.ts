...
describe('pageHits', () => {
  ...
  it('leaves the settings pages as pages', () => {
    const credentials = hits.find((hit) => hit.href === '/settings/credentials')!;
    expect(credentials.kind).toBe('page');
    expect(credentials.title).toBe('Credentials');
  });
...
describe('rankHits', () => {
  ...
  it('rests on the top of each family and shows a step once typed', () => {
    ...
    expect(resting).toContain('/settings/credentials');
...
describe('the palette in the window', () => {
  ...
  it('opens from the title bar and the Go menu', () => {
    expect(TITLE_BAR).toContain('usePalette');
    expect(MENU).toContain('PALETTE_COMMAND');
    expect(PROVIDER).toContain('onPalette(');
  });

  it('leaves the rail and the title bar in view', () => {
    expect(DIALOG).toContain('left-[var(--rail-w)]');
    expect(DIALOG).toContain('top-[var(--titlebar-h)]');
    expect(DIALOG).not.toContain('inset-0');
  });
