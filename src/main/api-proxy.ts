...
export function registerApiProxy(sidecar: Sidecar): void {
  ipcMain.handle(
    'api:request',
    async (_event, path: unknown, init: unknown): Promise<ApiResult> => {
  ...
  ipcMain.handle(
    'api:stream',
    async (event, path: unknown, init: unknown, streamId: unknown): Promise<ApiResult> => {
      if (typeof path !== 'string' || !path.startsWith('/api/')) {
        return { status: 400, body: { error: 'path must start with /api/' } };
      }
      if (!rendererMayCall(path)) {
      ...
      try {
        ...
        if (!response.ok || !response.body) {
          return {
            ...
            body: await response.json().catch(() => ({ error: 'stream failed' })),
          };
        }
