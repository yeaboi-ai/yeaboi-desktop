...
export function TicketWorkspace({ idOrKey, mode = 'page' }: Props) {
  ...
  if (error) {
    return <div className="p-6 text-sm text-destructive">Failed to load ticket: {error}</div>;
  }
  if (!ticket || !card) {
...
function TicketWorkspaceInner({
  ...
  const editingBanner = editing ? (
    ...
    >
      {editing.user.name ?? editing.user.email ?? 'Someone'} is editing this ticket. Yours will save
      normally; expect a merge if both finish at once.
    </div>
  ) : null;
  ...
  const mainEntries = layout.filter((e) => e.placement === 'main' && (layoutEditing || e.visible));
  ...
  const layoutEditingBanner = layoutEditing ? (
    <div className="text-xs rounded-md border border-primary/30 bg-primary/5 px-3 py-2 text-foreground flex items-center justify-between">
      <span>
        <span className="font-medium">Editing layout</span> — drag fields to reorder, rename labels
        inline, toggle required, hide built-ins, drop a field across columns to move it. Changes
        apply to every ticket on this template.
      </span>
      {layoutEditor.saving && (
    ...
    <div className={bodyClass}>
      {mode === 'page' ? (
        <>
          ...
          {detailsOpen && <div className="space-y-8 min-w-0">{sidebarRendered}</div>}
        </>
      ) : isWidePanel ? (
        <>
          ...
          {detailsOpen && <aside className="space-y-6 min-w-0">{sidebarRendered}</aside>}
        </>
      ) : (
        <>
          {editingBanner}
