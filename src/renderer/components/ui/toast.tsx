...
export const toast = {
  ...
  success: (opts: Omit<ShowOptions, "variant">) =>
    toast.show({ ...opts, variant: "success" }),
  warning: (opts: Omit<ShowOptions, "variant">) =>
    toast.show({ ...opts, variant: "warning" }),
  error: (opts: Omit<ShowOptions, "variant">) =>
    toast.show({ ...opts, variant: "destructive" }),
...
const VARIANT_STYLES: Record<ToastVariant, { ring: string; icon: React.ReactNode | null }> = {
  ...
  destructive: { ring: "ring-destructive/30", icon: <XCircle className="h-4 w-4 text-destructive" /> },
}
