...
export function CommandDialog({
  ...
  const visibleIds = React.useMemo(() => {
    ...
    return all.filter(([, value]) => value.toLowerCase().includes(q)).map(([id]) => id);
  }, [query, items]);
  ...
  function onKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      const item = document.querySelector<HTMLDivElement>(
        `[data-command-item-id="${highlightedId}"]`,
      );
      item?.click();
    }
  }
...
export function CommandGroup({
  ...
  return (
    <div role="group" className="py-1">
      {heading && (
        <div className="px-3 py-1 text-[10px] uppercase tracking-[0.08em] text-muted-foreground/50 font-medium">
          {heading}
        </div>
      )}
      {children}
...
export function CommandItem({ value, onSelect, children, shortcut, disabled }: CommandItemProps) {
  ...
  return (
    ...
    >
      ...
      {shortcut && (
        <span className="text-[10px] text-muted-foreground/50 font-mono tracking-wide shrink-0">
          {shortcut}
        </span>
      )}
    </div>
