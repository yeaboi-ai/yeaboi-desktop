...
// terminal cannot see a modifier chord on macOS and cannot detect key-release
// at all. Neither constraint exists here, so this is a button — press to
// start, press again to stop, Escape to throw the take away. That is the one
// place the two surfaces deliberately differ.
//
...
export function MicButton({ onText, disabled }: MicButtonProps) {
  ...
  async function begin(): Promise<void> {
    ...
    try {
      ...
      setPhase('idle');
      // The browser's own reason, not a house one: "denied", "in use by
      // another application" and "no such device" want three different
      // responses, and a single "could not access microphone" told you which
  ...
  async function finish(): Promise<void> {
    ...
    try {
      ...
      setPhase('idle');
      // No quip here on purpose. The words landing in the box are the
      // feedback; a duck congratulating every sentence would be the ambient
      // chatter the voice deliberately has no tier for.
      if (text) onText(text);
      ...
      <span className="inline-flex items-center gap-2">
        ...
        {phase === 'recording' && (
          <span className="text-[11px] font-mono text-destructive">
            {clock(elapsed)} <span className="tracking-tighter">{bar}</span>{' '}
        ...
        {phase === 'transcribing' && (
          <span className="text-[11px] text-muted-foreground">
            {transcribingLine(status?.model_cached ?? true)}
          </span>
        )}
        {phase === 'idle' && note && (
