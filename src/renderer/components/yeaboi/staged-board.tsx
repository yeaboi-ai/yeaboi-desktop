...
/* The ground, and only where there is ground to paint. The rule above hands
   out names, which a portal needs; this paints, which a portal must not do —
   the drag layer is fixed to the whole viewport, so a background on it is a
   sheet pulled over the window the moment a card leaves the stack. */
.board-frame .${HOST} {
  color: var(--text);
  background: var(--bg);
