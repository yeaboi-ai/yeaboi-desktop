    ...
    >
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {showSuggested && (
            <span className="text-[9px] font-normal normal-case tracking-normal text-info/60">
              AI suggested
            </span>
          )}
        </div>
      ...
      {hasContent ? (
        content.includes('\n- ') || content.startsWith('- ') ? (
          <ul
            className={`text-sm space-y-1 list-none ${showSuggested ? 'text-muted-foreground' : ''}`}
          >
            {content
              .split('\n')
              .filter(Boolean)
          ...
          <p
            className={`text-sm whitespace-pre-wrap ${showSuggested ? 'text-muted-foreground' : ''}`}
          >
            {content}
          </p>
        )
        ...
        <p
          className={`text-sm italic ${showWarning ? 'text-warning/70' : 'text-muted-foreground'}`}
        >
