...
function WorldRow({
  ...
  return (
    <button
      ...
      style={{ boxShadow: active ? `inset 2px 0 0 0 ${world.accentBright}` : 'none' }}
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5">
          ...
          {active && (
            <Check className="ml-auto h-3 w-3 shrink-0" style={{ color: world.accentBright }} />
          )}
        </span>
        <span className="mt-0.5 block font-body text-[11px] text-foreground/80">{world.verb}</span>
        <span className="mt-1 block font-body text-[10px] leading-relaxed text-muted-foreground">
...
export function WorldSwitcher({ onSwitch }: { onSwitch: (audience: Audience) => void }) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      {/* Nudged so the row mascots stand over the lockup's own, rather than
