...
const TASK_MODELS: {
  ...
  { key: 'flow_model', label: 'User-flow diagrams', hint: 'Step-by-step product flow generation' },
  {
    ...
    hint: 'Infrastructure / system diagrams',
  },
  {
    key: 'wireframe_model',
...
export function AIProviderSection() {
  ...
  const summary = useMemo(() => {
    switch (provider) {
      ...
      case 'byok': {
        const modelLabel =
          BYOK_MODELS[byokProvider]?.find((m) => m.id === byokDefaultModel)?.label ??
          'tier defaults';
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-border/40 bg-card/40 px-4 py-3">
        ...
        >
          <p className="text-[10px] text-muted-foreground font-body">
            Choose which model powers each heavy task. Auto uses the platform default. Applies to
            all providers above.
          </p>
          {TASK_MODELS.map((t) => (
