...
import { Settings2, Lightbulb, ChevronRight, ChevronDown, Globe, MessageSquare, FileText, Bug } from "lucide-react";
...
const PERSONAS = [
  { id: "default", label: "Senior Engineer", description: "Drives technical decisions", icon: "\u{1F6E0}" },
  { id: "pm", label: "Product Manager", description: "User-focused, impact-driven", icon: "\u{1F4CB}" },
  { id: "architect", label: "System Architect", description: "Scalability & patterns", icon: "\u{1F3D7}" },
  { id: "mentor", label: "Patient Mentor", description: "Explains & teaches", icon: "\u{1F393}" },
  { id: "challenger", label: "Devil's Advocate", description: "Stress-tests ideas", icon: "\u{1F525}" },
] as const;
...
const PACE_LEVELS = [
  ...
  { value: "balanced" as const, label: "Balanced", description: "5 questions / persona, 2 follow-ups" },
...
const TECHNICAL_COMFORT_LEVELS = [
  { value: "non_technical" as const, label: "Plain", description: "Explain technical terms in plain English" },
  { value: "comfortable" as const, label: "Comfortable", description: "I know the basics — only explain when I ask" },
  { value: "expert" as const, label: "Expert", description: "Skip definitions, go straight to trade-offs" },
];
...
export function AISettingsDrawer({ aiConfig, onConfigChange, aiSpeaking: _aiSpeaking = false, open, onToggle, personaRecommendations, inCall, chatOpen, onToggleChat, contextOpen, onToggleContext, hidden }: AISettingsDrawerProps) {
  const { authFetch, ready } = useAuthFetch();
  const orgId = getStoredOrgId();
  const [personas, setPersonas] = useState<Array<{id: string; slug: string; name: string; video_avatar_id?: string | null}>>([]);
  ...
  const currentPace = PACE_LEVELS.find((l) => l.value === (aiConfig.pace ?? "balanced")) ?? PACE_LEVELS[1];
  const currentTechnicalComfort =
    TECHNICAL_COMFORT_LEVELS.find((l) => l.value === (aiConfig.technical_comfort ?? "comfortable"))
        ...
        >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-border/60">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground font-medium">Settings</span>
              </div>
              <button
                onClick={onToggle}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-x divide-white/[0.05] max-h-[440px]">
              ...
              <section className="px-5 py-4 space-y-3 overflow-y-auto">
                <div className="flex items-center justify-between">
                  <Label className="text-[10px] text-muted-foreground/70 uppercase tracking-[0.14em]">Persona</Label>
                  {topRec && (
                    <button
                    ...
                    >
                      ...
                      <span className="text-[10px] font-medium text-amber-200/85">+{topRec.gaps_count}</span>
                    </button>
                  )}
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <span className={`text-[12px] font-medium block leading-tight ${active ? "text-foreground" : "text-muted-foreground"}`}>{persona.label}</span>
                          <span className="text-[10px] text-muted-foreground/50 block leading-tight">{persona.description}</span>
                        </div>
                        {hasGaps && !active && (
              ...
              <section className="px-5 py-4 space-y-4 overflow-y-auto">
                <div className="space-y-2">
                  <Label className="text-[10px] text-muted-foreground/70 uppercase tracking-[0.14em]">Involvement</Label>
                  <div
                  ...
                  <p className="text-[10px] text-muted-foreground/50 leading-snug">{currentInvolvement.description}</p>
                </div>

                <div className="border-t border-border/50" />

                <div className="space-y-2">
                  <Label className="text-[10px] text-muted-foreground/70 uppercase tracking-[0.14em]">Assertiveness</Label>
                  ...
                  <p className="text-[10px] text-muted-foreground/50 leading-snug">{currentAssertiveness.description}</p>
                </div>

                <div className="border-t border-border/50" />

                <div className="space-y-2">
                  <Label className="text-[10px] text-muted-foreground/70 uppercase tracking-[0.14em]">Pace</Label>
                  <div
                    className="flex gap-1 bg-foreground/[0.04] rounded-md p-1"
                    role="radiogroup"
                  ...
                  <p className="text-[10px] text-muted-foreground/50 leading-snug">{currentPace.description}</p>
                </div>

                <div className="border-t border-border/50" />

                <div className="space-y-2">
                  <Label className="text-[10px] text-muted-foreground/70 uppercase tracking-[0.14em]">Technical comfort</Label>
                  <div
                    className="flex gap-1 bg-foreground/[0.04] rounded-md p-1"
                    role="radiogroup"
                  ...
                  <p className="text-[10px] text-muted-foreground/50 leading-snug">{currentTechnicalComfort.description}</p>
                </div>

                <div className="border-t border-border/50" />

                <div className="space-y-2">
                  <Label className="text-[10px] text-muted-foreground/70 uppercase tracking-[0.14em]">UI Library</Label>
                  <div className="flex gap-1 bg-foreground/[0.04] rounded-md p-1">
                    {[
                      { value: "shadcn" as const, label: "shadcn", description: "Linear/Stripe-grade primitives. Recommended for vibe-coded MVPs." },
                      { value: "custom" as const, label: "Custom", description: "Bespoke Tailwind generated from scratch. More variety, less consistency." },
                    ].map((opt) => {
