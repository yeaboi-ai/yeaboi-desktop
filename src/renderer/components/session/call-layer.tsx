...
function AgentMuteBadge({ participant, className }: { participant: Participant; className: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center w-6 h-6 rounded-md transition-colors ${
        enabled ? "text-muted-foreground/70 hover:text-foreground/80 hover:bg-foreground/[0.06]" : "text-muted-foreground/40 hover:text-muted-foreground/80 hover:bg-foreground/[0.05]"
      }`}
    >
...
function useCallControls(onMicChange?: (muted: boolean) => void) {
  ...
  const applyCameraEffect = useCallback(async (bg: string) => {
    try {
  ...
  return { localParticipant, micEnabled, camEnabled, screenEnabled, toggleMic, toggleCam, toggleScreen, applyCameraEffect };
}

/* ── Control button ──────────────────────────────────────────────── */

function CtrlBtn({
  onClick, active, danger, label, children, size = "normal",
}: {
  onClick: () => void; active?: boolean; danger?: boolean; label?: string; children: React.ReactNode; size?: "compact" | "normal";
}) {
  ...
  return (
    ...
    >
      {children}
      {label && !isCompact && <span className="text-[8px] font-medium leading-none opacity-70">{label}</span>}
    </button>
  );
}
    ...
    <div className="shrink-0 border-b border-border/70 bg-gradient-to-r from-white/[0.02] to-white/[0.01]">
      <div className="flex items-center gap-2 px-3 py-2.5">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <span className="text-[10px] font-body tabular-nums text-muted-foreground font-medium">{timer}</span>
        </div>
        ...
        <div className="flex items-center gap-1 flex-1 min-w-0 overflow-hidden">
          {participants.map((p) => {
            const avatarUrl = (() => { try { return JSON.parse(p.metadata || "{}").avatar_url; } catch { return ""; } })();
            return (
            <div
              key={p.sid}
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <CtrlBtn onClick={toggleMic} active={micEnabled} label={micEnabled ? "Mute" : "Unmute"} size="compact">
            {micEnabled ? <Mic className="h-3.5 w-3.5 pointer-events-none" /> : <MicOff className="h-3.5 w-3.5 pointer-events-none" />}
          </CtrlBtn>
          <CtrlBtn onClick={toggleCam} active={camEnabled} label={camEnabled ? "Camera off" : "Camera on"} size="compact">
            {camEnabled ? <Video className="h-3.5 w-3.5 pointer-events-none" /> : <VideoOff className="h-3.5 w-3.5 pointer-events-none" />}
          </CtrlBtn>
  ...
  return createPortal(
    <div
      className={`fixed flex flex-col bg-background/97 backdrop-blur-2xl overflow-hidden ${
        fullscreen ? "rounded-none border-none" : "rounded-2xl border border-border shadow-[0_24px_80px_-12px_rgba(0,0,0,0.8)]"
      }`}
      style={{
        left: pos.x,
        top: pos.y,
        ...
        transition: dragging.current || resizing.current ? "none" : "width 0.3s, height 0.3s, border-radius 0.3s",
      }}
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider pointer-events-none">Meeting</span>
        </div>
      ...
      <div className="relative flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <VideoTileStatus micMuted={micMuted} pushToTalk={pushToTalk} captionsOn={captionsOn} isRecording={isRecording} agentInCall={agentInCall} />
        <ReactionsOverlay />
      ...
      >
        <svg className="w-3 h-3 text-muted-foreground/30 absolute bottom-1 right-1" viewBox="0 0 10 10" fill="currentColor">
...
function MoreMenu({
  ...
  inviteAI, agentStatus, aiCallPhase,
  fileInputRef,
  customBgs, saveCustomBg, deleteCustomBg,
  teamMembers,
  personaName, personaVideoPreviewUrl, personaSlug,
  micMuted, onToggleMic, pushToTalk,
  captionsOn, onToggleCaptions,
}: {
  showBgPicker: boolean; setShowBgPicker: (v: boolean) => void;
  tileBg: string; selectBg: (v: string) => void;
  camEnabled: boolean;
  showInvite: boolean; setShowInvite: (v: boolean) => void;
  inviteAI: () => void; agentStatus?: string; aiCallPhase: string;
  fileInputRef: React.RefObject<HTMLInputElement | null>;
  customBgs: string[]; saveCustomBg: (url: string) => void; deleteCustomBg: (url: string) => void;
  teamMembers: { id: string; name: string | null; email: string; avatar_url?: string | null }[];
  personaName: string;
  personaVideoPreviewUrl?: string | null;
  personaSlug?: string | null;
  ...
  return (
    <div className="relative" ref={ref}>
      <CtrlBtn onClick={() => { if (open) { setShowBgPicker(false); setShowInvite(false); } setOpen((v) => !v); }} active={open} label="More">
      ...
      {open && !showBgPicker && !showInvite && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 rounded-2xl border border-border/70 bg-popover shadow-2xl w-[220px] z-50 overflow-hidden py-1">
          ...
          {onToggleMic && (
            <button
              onClick={() => { onToggleMic(); setOpen(false); }}
              disabled={!!pushToTalk}
            ...
            >
              ...
              <span className="flex-1 text-left">
                <span className="block text-[12px] text-foreground/90 font-medium">
                  {pushToTalk ? "Push-to-talk" : micMuted ? "Resume AI listening" : "Pause AI listening"}
                </span>
          ...
          {onToggleCaptions && (
            ...
            >
              <Captions className={`h-4 w-4 pointer-events-none ${captionsOn ? "text-info" : "text-muted-foreground"}`} />
              <span className="flex-1 text-left">
                <span className="block text-[12px] text-foreground/90 font-medium">
                  {captionsOn ? "Hide captions" : "Show captions"}
                </span>
                <span className="block text-[10px] text-muted-foreground/70">Live subtitles over the call</span>
              </span>
            </button>
          )}
          ...
          >
            ...
            {tileBg !== "default" && <span className="w-1.5 h-1.5 rounded-full bg-primary ml-auto" />}
          </button>
          <button
      ...
      {open && showBgPicker && (
        <>
          <BgPicker
            current={tileBg}
            onSelect={(bg) => { selectBg(bg); setShowBgPicker(false); setOpen(false); }}
      ...
      {open && showInvite && inviteAnchorRect && createPortal(
        <div
          ref={invitePopupRef}
          ...
          style={{
            ...
            left: Math.max(8, Math.min(window.innerWidth - 268, inviteAnchorRect.left + inviteAnchorRect.width / 2 - 130)),
        ...
        >
          <div className="px-4 pt-3 pb-2 flex items-center justify-between shrink-0">
            ...
            <button onClick={() => setShowInvite(false)} className="text-[10px] text-muted-foreground/50 hover:text-muted-foreground">Back</button>
          </div>
              ...
              teamMembers.map((m) => (
                ...
                >
                  ...
                  <div className="flex-1 text-left min-w-0">
                    <span className="text-[13px] text-foreground/80 font-medium block leading-tight truncate">{m.name || m.email}</span>
                    <span className="text-[11px] text-muted-foreground/40 leading-tight truncate block">{m.email}</span>
                  </div>
                  <span className="text-[9px] text-muted-foreground/30 shrink-0">Coming soon</span>
                </div>
              ))
            ) : (
              <p className="text-[11px] text-muted-foreground/50 px-3 py-3 text-center">No teammates yet</p>
            )}
          </div>
        </div>,
        document.body
...
const BG_OPTIONS = [
  ...
  { id: "gradient-sunset", label: "Sunset", style: "linear-gradient(135deg, #1a1a1a, #2d1b30, #1a1a1a)" },
  { id: "gradient-forest", label: "Forest", style: "linear-gradient(135deg, #0a1a0a, #1a2a1a)" },
  { id: "gradient-aurora", label: "Aurora", style: "linear-gradient(135deg, #0f0c29, #302b63, #24243e)" },
  { id: "blur-light", label: "Blur (light)", style: "blur-light" },
  { id: "blur-heavy", label: "Blur (heavy)", style: "blur-heavy" },
  { id: "img-office", label: "Office", style: "https://images.unsplash.com/photo-1497366216548-37526070297c?w=600&q=60" },
  { id: "img-nature", label: "Nature", style: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=600&q=60" },
  { id: "img-space", label: "Space", style: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=600&q=60" },
  { id: "img-city", label: "City", style: "https://images.unsplash.com/photo-1480714378408-67cf0d13bc1b?w=600&q=60" },
];
...
function BgPicker({
  ...
  return (
    <div ref={ref} className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 rounded-2xl border border-border/70 bg-popover shadow-2xl w-[300px] z-50 overflow-hidden">
        ...
        {customBgs.map((url, i) => {
          ...
          return (
            <div key={`custom-${i}`} className="relative group">
              ...
              {onDeleteCustom && (
                <button
                  onClick={(e) => { e.stopPropagation(); onDeleteCustom(url); }}
...
function VideoPanel({
  ...
  const { localParticipant } = useLocalParticipant();
  const { micEnabled, camEnabled, screenEnabled, toggleMic, toggleCam, toggleScreen, applyCameraEffect } = useCallControls(onMicMuteChange);
  ...
  const [tileBg, setTileBg] = useState(() => {
    try { return localStorage.getItem("planr-call-bg") || "default"; } catch { return "default"; }
  });
  const [customBgs, setCustomBgs] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem("planr-custom-bgs") || "[]"); } catch { return []; }
  });
  const [aiCallPhase, setAiCallPhase] = useState<"idle" | "ringing" | "unavailable" | "joined">("idle");
  ...
  const selectBg = useCallback((bg: string) => {
    setTileBg(bg);
    try { localStorage.setItem("planr-call-bg", bg); } catch { /* quota */ }
  ...
  const saveCustomBg = useCallback((dataUrl: string) => {
    setCustomBgs((prev) => {
      ...
      try { localStorage.setItem("planr-custom-bgs", JSON.stringify(next)); } catch { /* quota */ }
      return next;
    });
  }, []);

  const deleteCustomBg = useCallback((dataUrl: string) => {
    setCustomBgs((prev) => {
  ...
  useEffect(() => { onAiRingingChangeRef.current = onAiRingingChange; }, [onAiRingingChange]);
  ...
  const inviteAI = useCallback(() => {
    ...
    try {
      ...
      const playChime = () => {
        ...
        setTimeout(() => { if (playing) playChime(); }, 3000);
      };
      playChime();
      ringOscRef.current = { stop: () => { playing = false; ctx.close(); } };
    } catch {
      // Audio not available
    }
  ...
  const agentSpeaking = aiCallPhase === "joined"
    && ((agentParticipant?.isSpeaking ?? false) || !!realtimeSpeaking);
  ...
  const cameraGridClass = `grid gap-2 flex-1 min-h-0 auto-rows-[1fr] ${
    (sortedParticipants.length + (aiCallPhase === "ringing" || aiCallPhase === "unavailable" || aiCallPhase === "joined" ? 1 : 0)) <= 1
      ? "grid-cols-1"
      : "grid-cols-2"
  }`;
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-gradient-to-b from-black/20 to-black/40">
      ...
      <div className={`p-3 flex-1 min-h-0 flex flex-col gap-2 ${isSharing ? "" : "justify-center"}`}>
        {isSharing && (
          <div className="flex-1 min-h-0 flex flex-col gap-2">
            {screenTracks.map((track) => {
              const isLocalShare = track.participant.sid === localParticipant?.sid;
              const presenterName = track.participant.name || track.participant.identity || "Someone";
              return (
                <div
                ...
                {hasActiveCamera && cameraTrack ? (
                  <>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent pt-8 pb-2.5 px-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[13px] text-foreground font-medium drop-shadow-lg">{name}</span>
                          {p.isSpeaking && (
                            <div className="flex items-center gap-1">
                              <span className="w-1 h-1 rounded-full bg-success animate-pulse" />
                              <span className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" style={{ animationDelay: "0.15s" }} />
                              <span className="w-1 h-1 rounded-full bg-success animate-pulse" style={{ animationDelay: "0.3s" }} />
                            </div>
                          )}
                        </div>
                        {isAgent ? (
                  ...
                  >
                    {avatarUrl ? (
                        ...
                        <img
                          ...
                          className={`w-full h-full rounded-full object-cover transition-all duration-500 ${
                            handIsRaised
                              ...
                              : p.isSpeaking ? "ring-[3px] ring-success/70 scale-105" : "ring-2 ring-white/15"
                          }`}
                        />
                      ...
                      <div className="relative" style={{ width: "min(30%, 120px)", aspectRatio: "1" }}>
                        <div className={`w-full h-full rounded-full flex items-center justify-center text-[clamp(1.2rem,3vw,2.5rem)] font-semibold transition-all duration-500 ${
                          handIsRaised
                          ...
                          {initials ? initials : <UserPlus className="h-[40%] w-[40%] text-muted-foreground/80" />}
                        </div>
                        {handIsRaised && (
                          <div
                            aria-label={`${name || "Participant"} has raised their hand`}
        ...
        {aiCallPhase === "ringing" && (
          <div className="rounded-xl overflow-hidden bg-card ring-1 ring-warning/20 flex flex-col items-center justify-center gap-4 shadow-lg min-h-0">
            <div className="relative" style={{ width: "min(30%, 120px)", aspectRatio: "1" }}>
              <div className="absolute inset-0 rounded-full bg-warning/20 animate-ping" style={{ animationDuration: "1.5s" }} />
            ...
            <div className="flex flex-col items-center gap-2">
              <span className="text-sm text-muted-foreground font-medium">Calling {personaName}...</span>
              <button
        ...
        {aiCallPhase === "unavailable" && (
          <div className="rounded-xl overflow-hidden bg-card ring-1 ring-destructive/20 flex flex-col items-center justify-center gap-4 shadow-lg min-h-0">
            <div className="rounded-full bg-destructive/10 flex items-center justify-center" style={{ width: "min(30%, 120px)", aspectRatio: "1" }}>
              <PhoneOff className="h-[40%] w-[40%] text-destructive/50 pointer-events-none" />
            </div>
            <div className="flex flex-col items-center gap-2">
              <span className="text-sm text-muted-foreground/70 font-medium">{personaName} unavailable</span>
              <div className="flex items-center gap-2">
                <button onClick={inviteAI} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-foreground/[0.08] text-muted-foreground hover:text-foreground text-xs font-medium transition-colors">
                  <RotateCcw className="h-3 w-3 pointer-events-none" />Retry
                </button>
                <button onClick={() => setAiCallPhase("idle")} className="px-4 py-2 rounded-xl text-muted-foreground/40 hover:text-muted-foreground/70 text-xs font-medium transition-colors">
                  Dismiss
                </button>
              </div>
            </div>
        ...
        {aiCallPhase === "joined" && (agentStatus === "connected" || agentStatus === "extracting" || agentStatus === "detaching") && (
          <div className={`group/aitile relative rounded-xl overflow-hidden bg-card shadow-lg min-h-0 transition-all duration-500 ${
            agentSpeaking
              ? "ring-2 ring-success/70 shadow-emerald-500/10"
            ...
            {showAgentVideo && agentCameraTrack ? (
              <>
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent pt-8 pb-2.5 px-3 pointer-events-none">
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] text-foreground font-medium drop-shadow-lg">{personaName}</span>
                    {agentSpeaking && (
                      <div className="flex items-center gap-1">
                        <span className="w-1 h-1 rounded-full bg-success animate-pulse" />
                        <span className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" style={{ animationDelay: "0.15s" }} />
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-4">
                <div className={`rounded-full overflow-hidden flex items-center justify-center transition-all duration-500 ${
                  ...
                  <PersonaThumbnail videoPreviewUrl={personaVideoPreviewUrl} slug={personaSlug} alt={personaName} expandable className="w-full h-full object-cover" />
                </div>
                <div className="flex flex-col items-center gap-1.5">
        ...
        <div className="border-t border-border/60 px-3 py-2 shrink-0 max-h-[200px] overflow-auto">
          <div className="space-y-1">
            {participants.map((p) => {
              ...
              return (
                <div key={p.sid} className="flex items-center gap-2.5 py-1.5 px-1 rounded-lg hover:bg-foreground/[0.05]">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[9px] font-medium overflow-hidden shrink-0 ${
                    ...
                    {avatarUrl ? (
                      ...
                      <div className={`w-full h-full flex items-center justify-center ${
                        p.isSpeaking ? "bg-success/25 text-success" : "bg-foreground/[0.10] text-muted-foreground"
                      }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-[11px] text-foreground/80 font-medium truncate block">{name}</span>
            ...
            {aiCallPhase === "joined" && (agentStatus === "connected" || agentStatus === "extracting" || agentStatus === "detaching") && (
              <div className="group flex items-center gap-2.5 py-1.5 px-1 rounded-lg hover:bg-foreground/[0.04]">
                ...
                />
                <div className="flex-1 min-w-0">
                  <span className="text-[11px] text-foreground/80 font-medium">{personaName}</span>
                  {agentStatus === "extracting" && <span className="text-[9px] text-info block">Updating blueprint...</span>}
                  {agentStatus === "detaching" && <span className="text-[9px] text-warning block">Leaving…</span>}
                </div>
                {onDetachAgent && agentStatus !== "detaching" && (
                  <button
                    onClick={onDetachAgent}
      ...
      <div className="flex flex-col gap-3 px-4 py-4 border-t border-border/70 bg-background/40 shrink-0">
        ...
        <div className="flex items-center justify-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-foreground/[0.05]">
            ...
            <span className="text-xs font-body tabular-nums text-muted-foreground font-medium">{timer}</span>
          </div>
          <button
          ...
          >
            ...
            {participants.length + (aiCallPhase === "joined" && (agentStatus === "connected" || agentStatus === "extracting" || agentStatus === "detaching") ? 1 : 0)}
          </button>
          {aiCallPhase !== "joined" && agentStatus !== "connected" && agentStatus !== "ready" && agentStatus !== "extracting" && agentStatus !== "detaching" && (
            <button
              onClick={() => { if ((aiCallPhase === "idle" || aiCallPhase === "unavailable") && !callBlocked) inviteAI(); }}
              disabled={aiCallPhase === "ringing" || callBlocked}
              title={callBlocked ? `AI agent paused: ${videoHealth.message ?? "an upstream provider is unavailable."}` : undefined}
            ...
            >
              <Phone className={`h-3 w-3 pointer-events-none ${aiCallPhase === "ringing" ? "animate-pulse" : ""}`} />
              {callBlocked ? "AI paused" : aiCallPhase === "ringing" ? "Ringing…" : aiCallPhase === "unavailable" ? "Retry AI" : "Call AI"}
            </button>
          )}
        </div>
        ...
        <div className="flex items-center justify-center gap-2.5">
          <CtrlBtn onClick={toggleMic} active={micEnabled} label={micEnabled ? "Mute" : "Unmute"}>
            {micEnabled ? <Mic className="h-5 w-5 pointer-events-none" /> : <MicOff className="h-5 w-5 pointer-events-none" />}
          </CtrlBtn>
          <CtrlBtn onClick={toggleCam} active={camEnabled} label={camEnabled ? "Stop Video" : "Video"}>
            {camEnabled ? <Video className="h-5 w-5 pointer-events-none" /> : <VideoOff className="h-5 w-5 pointer-events-none" />}
          </CtrlBtn>
          <CtrlBtn
            onClick={toggleScreen}
          ...
          >
            {screenEnabled ? <MonitorOff className="h-5 w-5 pointer-events-none" /> : <MonitorUp className="h-5 w-5 pointer-events-none" />}
          </CtrlBtn>
          {isSharing && (
            <CtrlBtn
