    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Backdrop
          className="fixed inset-0 z-[280] bg-background/70 backdrop-blur-sm data-[starting-style]:opacity-0 data-[ending-style]:opacity-0 transition-opacity duration-150"
        />
        <DialogPrimitive.Popup
          className="fixed top-1/2 left-1/2 z-[290] -translate-x-1/2 -translate-y-1/2 w-[min(560px,calc(100vw-2rem))] rounded-2xl bg-card ring-1 ring-border/70 shadow-2xl outline-none data-[starting-style]:opacity-0 data-[starting-style]:scale-95 data-[ending-style]:opacity-0 data-[ending-style]:scale-95 transition-[opacity,transform] duration-150"
        >
            ...
            {order
              ...
              .map((g) => (
                <div key={g} className="space-y-2">
                  <p className="text-[10px] uppercase tracking-[0.08em] text-muted-foreground/70 font-medium">{g}</p>
          ...
          <div className="px-5 py-3 border-t border-border/60 text-[11px] text-muted-foreground/70">
            Press <kbd className="font-mono bg-foreground/[0.06] border border-border/70 rounded px-1.5 py-0.5">?</kbd> any time to open this list.
          </div>
        </DialogPrimitive.Popup>
      </DialogPrimitive.Portal>
