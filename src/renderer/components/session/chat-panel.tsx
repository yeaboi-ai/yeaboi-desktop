...
interface ChatPanelProps {
  ...
  ) => { videoPreviewUrl: string | null; slug: string } | null;
  ...
  return (
    <div className="flex items-center gap-2.5 mb-1.5">
      ...
      <div className="flex-1 min-w-0 flex items-center gap-2.5">
        <div className="flex-1 h-6 flex items-center cursor-pointer group" onClick={seek}>
          <div className="w-full h-1.5 rounded-full bg-foreground/[0.10] overflow-hidden relative">
            <div
              className={`h-full rounded-full ${playing ? 'bg-success/70' : 'bg-white/25'}`}
...
function PreviewPlayer({
  ...
  return (
    <div className="border-t border-border/60 p-3 flex items-center gap-2 shrink-0">
      ...
      <span className="text-xs text-muted-foreground/70 tabular-nums flex-1">
        {formatTime(duration)}
      </span>
      <button
        onClick={onSend}
...
export function ChatPanel({
  ...
  resolvePersonaAvatar,
  userAvatarUrl,
  sectionUpdatesByMessageId,
  undoTargetByMessageId,
  onUndoAiUpdate,
  onLoadOlderMessages,
  typingUsers,
  onTypingSignal,
  ...
  const [mentionState, setMentionState] = useState<{ startIdx: number; query: string } | null>(
    null,
  );
  const [mentionSelectedIdx, setMentionSelectedIdx] = useState(0);
  const filteredMentions = (
    mentionState && mentionableParticipants
  ...
  const storageKey =
    typeof window !== 'undefined' ? `chat-history:${window.location.pathname}` : '';
  const sentHistory = useRef<string[]>([]);
  const historyLoaded = useRef(false);
  if (!historyLoaded.current && typeof window !== 'undefined') {
    try {
      sentHistory.current = JSON.parse(sessionStorage.getItem(storageKey) || '[]');
    } catch {
      /* ignore */
    }
    ...
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === 'Enter' && !e.shiftKey) {
        ...
        if (input.trim()) {
          ...
          try {
            sessionStorage.setItem(storageKey, JSON.stringify(sentHistory.current));
          } catch {
            /* ignore */
          }
  ...
  return (
    <GlossaryProvider
      onAskAi={(term) => {
        ...
        onSend(`Tell me more about "${term}" and how it applies to what we're planning.`);
      }}
    >
      <div
      ...
      >
          ...
          <button
            ...
            aria-label={
              newSinceScrolledUp > 0
                ? `Jump to ${newSinceScrolledUp} new messages`
                : 'Jump to latest'
            }
          >
            {newSinceScrolledUp > 0 && (
          ...
          {messages.map((msg) => {
            ...
            return isSystem ? (
              ...
              >
                ...
                <span
                  className="text-muted-foreground/40 shrink-0 text-center"
                  dangerouslySetInnerHTML={{
                    __html: (msg.content || '').replace(
                      /\*\*(.+?)\*\*/g,
                      '<span class="text-muted-foreground font-semibold">$1</span>',
                    ),
                  }}
                />
                <div className="flex-1 border-t border-border/60" />
                ...
                <div className={`group/msg flex gap-2.5 ${isUser ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`min-w-0 max-w-[85%] ${isAi ? '' : 'flex flex-col items-end'}`}>
                    <div className="flex items-end gap-1.5">
                      <div
                        className={`relative break-words overflow-hidden text-sm rounded-xl p-3 ${
                          isAi
                      ...
                      >
                        ...
                        {isAi &&
                          (() => {
                            const aiMeta = extractAIMeta(
                              msg.attachments as unknown as
                            ...
                            return (
                              <div className="flex items-center gap-1 mb-1">
                                <span className="text-[10px] font-medium text-success/80">
                                  {msg.speaker_name ||
                                    PERSONA_LABELS[persona || 'default'] ||
                                    'AI Facilitator'}
                                </span>
                        ...
                        {msg.attachments
                          ?.filter((att) => !!(att as unknown as Record<string, unknown>)?.url)
                          .map((att, i) => (
                                    ...
                                    <ReactMarkdown
                                      components={
                                        isAi
                                          ? {
                                              p: ({ children }) => (
                                                <p>{highlightChildren(children, seenTerms)}</p>
                                              ),
                                              li: ({ children }) => (
                                                <li>{highlightChildren(children, seenTerms)}</li>
                                              ),
                                              strong: ({ children }) => (
                                                <strong>
                                                  {highlightChildren(children, seenTerms)}
                                                </strong>
                                              ),
                                              em: ({ children }) => (
                                                <em>{highlightChildren(children, seenTerms)}</em>
                                              ),
                                            }
                                          : undefined
                                      }
                  ...
                  triggered one or more blueprint section updates. */}
                    {(() => {
                      const updatedSections = isAi
                        ? sectionUpdatesByMessageId?.[msg.id]
                        : undefined;
                    ...
                    {!readOnly && onToggleReaction && msg.message_type !== 'system' && (
                      <div
                        className={`relative flex items-center gap-1 mt-1 ${isUser ? 'flex-row-reverse' : ''}`}
                      >
                        {Object.entries(msg.reactions || {}).map(([emoji, userIds]) => {
                          ...
                          return (
                            <button
                              ...
                              title={
                                reactedByMe ? 'Remove your reaction' : `${userIds.length} reacted`
                              }
                            >
                              <span>{emoji}</span>
                    ...
                    {msg.created_at && (
                      <span
                        className={`text-[9px] text-muted-foreground/30 mt-0.5 px-1 ${isUser ? 'text-right' : ''}`}
                      >
                        {formatTimestamp(msg.created_at)}
                        {msg.original_content && (
                          <span className="ml-1 text-muted-foreground/30">(edited)</span>
                        )}
                      </span>
                    )}
                  </div>
        ...
        {messages.length === 0 && !input && !pendingFile && recorder.state === 'idle' && (
          <div className="px-3 py-2 border-t border-border/40">
            <span className="text-[9px] text-muted-foreground/30 uppercase tracking-wider font-medium block mb-1.5">
              Try one of these
            </span>
        ...
        {typingUsers && typingUsers.length > 0 && (
          <div
            className="px-4 py-1 text-[10px] text-muted-foreground/60 italic shrink-0"
            aria-live="polite"
          >
        ...
        {recorder.state === 'recorded' && recorder.audioUrl ? (
          ...
          <div className="relative border-t border-border/60 shrink-0">
                ...
                {filteredMentions.map((participant, i) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <span className="text-[11px] text-foreground/80 block">
                        {participant.name}
                      </span>
                      <span className="text-[9px] text-muted-foreground/40 font-mono">
                        @{participant.name.split(' ', 1)[0].toLowerCase()}
                      </span>
                    </div>
                  </button>
                ))}
            ...
            {showCommands && filteredCommands.length > 0 && !subOptions && (
              <div
                ref={commandListRef}
                className="absolute bottom-full left-0 right-0 mb-0 mx-2 bg-secondary border border-border rounded-xl shadow-2xl overflow-hidden z-50 max-h-[280px] overflow-y-auto"
              >
                <div className="px-3 py-1.5 border-b border-border/60">
                  <span className="text-[9px] font-medium text-muted-foreground/40 uppercase tracking-wider">
                    Commands
                ...
                {filteredCommands.map((cmd, i) => (
                  ...
                  >
                    <span className="text-xs text-success/70 font-mono shrink-0 mt-px">
                      /{cmd.name}
                    </span>
                    <div className="min-w-0">
            ...
            {readOnly && (
              <div className="p-3 text-center">
                <p className="text-[10px] font-body text-muted-foreground/40">
                  This release is finalized. Chat is view-only.
                </p>
              </div>
            )}
            {!readOnly && (
