...
function BoardPanelContent({
  ...
  const size = containerWidth >= 700 ? "full" : (containerWidth >= 350 || layout.h > 3) ? "normal" : "small";
    ...
    <div className="flex flex-wrap gap-x-3 gap-y-1 mb-4">
      {columns.map((col) => (
        <div key={col.name} className="flex items-center gap-1.5">
          <span className={`h-1.5 w-1.5 rounded-full ${COLUMN_COLORS[col.name] ?? "bg-muted-foreground/20"}`} />
          <span className="text-[11px] font-body text-muted-foreground/70">{col.name}</span>
          <span className="text-[11px] font-body tabular-nums text-muted-foreground/40">{col.cards.length}</span>
        </div>
      ))}
    </div>
  );
          ...
          {columns.map((col) => (
            <div key={col.name} className="flex-1 min-w-0 flex flex-col min-h-0">
              ...
              <div className="flex items-center gap-1.5 mb-2 pb-1.5 border-b border-border/30">
                <span className={`h-2 w-2 rounded-full shrink-0 ${COLUMN_COLORS[col.name] ?? "bg-muted-foreground/20"}`} />
                <span className="text-[11px] font-body font-medium text-muted-foreground truncate">{col.name}</span>
                <span className="text-[10px] font-body tabular-nums text-muted-foreground/40 ml-auto">{col.cards.length}</span>
              </div>
              {/* Cards */}
              <div className="space-y-1.5 flex-1 min-h-0">
                {col.cards.slice(0, maxCards).map((card) => (
                  ...
                  >
                    <span className="text-[11px] font-body text-foreground/80 line-clamp-2 leading-snug">{card.title}</span>
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      {card.labels?.slice(0, 2).map((l) => (
                        <span key={l} className="text-[8px] font-body text-muted-foreground/50 bg-muted/30 rounded px-1 py-px">{l}</span>
                      ))}
                      {card.story_points != null && (
                        <span className="ml-auto text-[9px] font-body tabular-nums text-muted-foreground/30">{card.story_points}</span>
                      )}
                    </div>
                  </Link>
                ))}
...
function AnalyticsPanelContent({
  ...
  const sparkline = timeline.length > 0 ? (
  ...
  const prioColors: Record<string, string> = {
    critical: "bg-red-500/80", high: "bg-orange-500/80", medium: "bg-[#e5a630]/80", low: "bg-success/60", none: "bg-muted-foreground/30",
  };
        ...
        <div className="grid grid-cols-5 gap-3 mb-6">
          {[
            ...
            <div key={label} className="border border-border/30 rounded-lg px-4 py-3">
              <p className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide mb-1.5">{label}</p>
              <span className="font-display text-3xl italic text-foreground leading-none">{value}</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 flex-1 min-h-0">
          ...
          <div className="flex flex-col">
            ...
            {eng && eng.total_committed > 0 ? (
              <div className="mb-5">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide">Delivery</span>
                  <span className="text-xs font-body text-success">{eng.total_done}/{eng.total_committed} ({Math.round(eng.delivery_rate)}%)</span>
                </div>
                <div className="h-3 bg-border rounded-full overflow-hidden">
                  <div className="h-full bg-success/70 rounded-full transition-all" style={{ width: `${eng.delivery_rate}%` }} />
                </div>
              </div>
            ) : (
              <div className="mb-5">
            ...
            {eng && eng.open_by_priority.length > 0 && (
              <div className="mb-5">
                <p className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide mb-3">Open by Priority</p>
                <div className="space-y-2.5">
                  {eng.open_by_priority.map((p) => {
                    const maxP = Math.max(...eng.open_by_priority.map(x => x.count), 1);
                    return (
                      <div key={p.priority} className="flex items-center gap-3">
                        <span className="text-xs font-body text-muted-foreground/60 capitalize w-16 shrink-0">{p.priority}</span>
                        <div className="flex-1 h-2 bg-border/40 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${prioColors[p.priority] || "bg-muted-foreground/30"}`} style={{ width: `${(p.count / maxP) * 100}%` }} />
                        </div>
                        <span className="text-xs font-body tabular-nums text-muted-foreground/50 w-6 text-right">{p.count}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-auto">
              {[
                ...
                <div key={label} className="border border-border/20 rounded-md px-3 py-2">
                  <p className="text-[9px] font-body text-muted-foreground/40 uppercase tracking-wide mb-0.5">{label}</p>
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-5">
              <p className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide mb-3">Sessions over time</p>
              {timeline.length > 0 ? (
                <>
                    ...
                    {timeline.map((point, i) => {
                      ...
                      return (
                        <div key={i} className="flex-1 group relative" style={{ height: "100%" }}>
                          <div className="absolute bottom-0 left-0 right-0 bg-[#e5a630]/50 hover:bg-[#e5a630] rounded-t-sm transition-colors" style={{ height: `${barH}%` }} />
                          <div className="absolute -top-5 left-1/2 -translate-x-1/2 hidden group-hover:block whitespace-nowrap bg-card border border-border rounded px-1 py-px text-[8px] font-body text-foreground z-10 pointer-events-none">
                            {point.sessions} &middot; {point.date}
                          </div>
                        </div>
                  ...
                  {timeline.length >= 2 && (
                    <div className="flex justify-between mt-1.5">
                      <span className="text-[9px] font-body text-muted-foreground/30">{timeline[0].date}</span>
                      <span className="text-[9px] font-body text-muted-foreground/30">{timeline[timeline.length - 1].date}</span>
                    </div>
                  )}
                </>
              ) : (
            ...
            {Object.keys(metrics.persona_distribution).length > 0 && (
              <div>
                <p className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide mb-3">Personas Used</p>
                <div className="space-y-2.5">
                  {Object.entries(metrics.persona_distribution).sort((a, b) => b[1] - a[1]).map(([name, count]) => {
                    const maxPersona = Math.max(...Object.values(metrics.persona_distribution), 1);
                    return (
                      <div key={name} className="flex items-center gap-3">
                        <span className="text-xs font-body text-muted-foreground/60 capitalize w-20 shrink-0 truncate">{name.replace(/_/g, " ")}</span>
                        <div className="flex-1 h-2 bg-border/40 rounded-full overflow-hidden">
                          <div className="h-full bg-[#e5a630]/60 rounded-full" style={{ width: `${(count / maxPersona) * 100}%` }} />
                        </div>
                        <span className="text-xs font-body tabular-nums text-muted-foreground/50 w-6 text-right">{count}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[
          ...
          <div key={label} className="border border-border/40 rounded-md px-3 py-2">
            <p className="text-[9px] font-body text-muted-foreground/50 uppercase tracking-wide mb-1">{label}</p>
            <span className={`font-display text-2xl italic leading-none ${accent ? "text-[#e5a630]" : "text-foreground"}`}>
              {value}
            </span>
          </div>
        ))}
      ...
      {eng && eng.total_committed > 0 && (
        <div className="mb-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[9px] font-body text-muted-foreground/50 uppercase tracking-wide">Delivery</span>
            <span className="text-[10px] font-body text-success">{Math.round(eng.delivery_rate)}%</span>
          </div>
    ...
    <div ref={containerRef}>
      ...
      <div className={isWide ? "grid grid-cols-2 gap-x-8 gap-y-1.5" : "space-y-0.5"}>
        {sections.map(({ key, label }) => {
          ...
          return (
            <div key={key} className={`flex items-center gap-2.5 px-1 ${isWide ? "py-1" : "py-0.5"}`}>
              <span className={`${isWide ? "w-2 h-2" : "w-1.5 h-1.5"} rounded-full shrink-0 ${
                isInherited
                  ? "bg-muted-foreground/15"
                  : content ? "bg-success/70" : "bg-primary/40"
              }`} />
              <span className={`font-body ${
                isInherited
                  ? "text-muted-foreground/25"
                  : content ? "text-foreground/60" : "text-muted-foreground/40"
              } ${isWide ? "text-xs" : "text-[11px]"}`}>{label}</span>
              {isInherited && (
      ...
      {expanded && (
        <div className={`mt-2 ${isWide ? "grid grid-cols-2 gap-3" : "space-y-1.5"}`}>
          {sections.map(({ key, label }) => {
            ...
            const isInherited = focusSections && !focusSections.has(key);
            return (
              <div key={key} className={`rounded-md px-3 py-2 ${isInherited ? "bg-white/[0.01] opacity-50" : "bg-white/[0.03]"}`}>
                <div className="flex items-center gap-2">
                  <span className={`font-body font-medium text-foreground/40 uppercase tracking-wider ${isWide ? "text-[10px]" : "text-[9px]"}`}>{label}</span>
                  {isInherited && <span className="text-[8px] font-body text-muted-foreground/30">inherited</span>}
                </div>
                <p className={`font-body text-foreground/70 leading-relaxed mt-0.5 ${isWide ? "text-xs line-clamp-5" : "text-[11px] line-clamp-3"}`}>
                  {content}
                </p>
              </div>
            );
...
function SessionItem({ session: s, projectId, isAdmin, onDelete, onRename, iterationLabel, isCompletingSession }: SessionItemProps) {
    ...
    <div className="relative group">
      ...
      >
            ...
            <>
              ...
              {iterationLabel && (
                <span
                  ...
                  title={isCompletingSession ? `Completed ${iterationLabel} — deleting this will remove ${iterationLabel} and all versions based on it` : `Contributed to ${iterationLabel}`}
                >
                  {iterationLabel}
                </span>
              )}
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {isActive && (
            <span className="text-[10px] font-body text-primary/70">Resume →</span>
          )}
...
export default function ProjectDetailPage({
  ...
  const [diagrams, setDiagrams] = useState<Array<{ session_id: string; session_title: string | null; diagram: { type: string; title?: string; nodes?: unknown[]; tables?: unknown[]; screens?: unknown[] }; created_at: string | null }>>([]);
  const [iterations, setIterations] = useState<Array<{ id: string; iteration_number: number; label: string; display_name?: string | null; status: string; iteration_type?: string | null; forked_from_id?: string | null }>>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      const [projectResp, sessionsResp, teamResp, blueprintResp, boardResp, iterResp, diagramsResp, analyticsResp, engResp] =
        await Promise.all([
      ...
      if (iterResp.ok) {
        ...
        const active = iters.find((i: { status: string }) => i.status === "planning") || iters[iters.length - 1];
      ...
      if (meResp.ok) {
        ...
        if (!admin) {
          const orgId = me.org_id || (typeof window !== "undefined" ? localStorage.getItem("current_org_id") : null);
          if (orgId) {
  ...
  const handleDeleteSession = useCallback(
    async (sessionId: string, e: React.MouseEvent) => {
      ...
      const completingIter = targetSession?.status === "completed" && targetSession?.iteration_id
        ? iterations.find((i) => i.id === targetSession.iteration_id && i.status === "locked")
        : null;

      const ok = await confirm({
        title: completingIter ? `Delete Session & ${releaseName(completingIter)}` : "Delete Session",
      ...
      if (resp.ok || resp.status === 204) {
        ...
        if (completingIter) {
          ...
          if (bpResp.ok) { const bp = await bpResp.json(); setBlueprint(bp.content || bp); }
          if (iterResp.ok) {
            const iters = await iterResp.json();
            setIterations(iters);
  ...
  const typeFilter = activeIter?.iteration_type && ITERATION_TYPE_SECTIONS[activeIter.iteration_type];
    ...
    <div className="min-h-screen">
      <main className="mx-auto max-w-6xl px-6 py-14">
        ...
        >
          ...
          {/* Panel: Releases */}
          <div key="sessions" className="h-full"><DashboardPanel label="Releases">
            {sessions.length === 0 ? (
              ...
              <div className="grid grid-cols-[1fr_auto] gap-6 mb-3">
                <div className="space-y-1">
                  {sessions.map((s) => {
                    const boundIter = s.iteration_id ? iterations.find((i) => i.id === s.iteration_id) : null;
                    const rName = boundIter ? releaseName(boundIter) : null;
                    const isCompleting = boundIter?.status === "locked" && s.status === "completed";
                    return (
                      <SessionItem
                        key={s.id}
                        session={s}
                ...
                <div className="flex flex-col gap-3 pr-2 shrink-0 min-w-[140px]">
                  <div className="border border-border/30 rounded-lg px-4 py-3">
                    <p className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide mb-1">Total</p>
                    <span className="font-display text-3xl italic text-foreground leading-none">{sessions.length}</span>
                  </div>
                  <div className="border border-border/30 rounded-lg px-4 py-3">
                    <p className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide mb-1">Active</p>
                    <span className="font-display text-3xl italic text-success leading-none">
                      {sessions.filter(s => s.status === "live" || s.status === "lobby").length}
                    </span>
                  </div>
                  <div className="border border-border/30 rounded-lg px-4 py-3">
                    <p className="text-[10px] font-body text-muted-foreground/50 uppercase tracking-wide mb-1">Completed</p>
          ...
          {/* Panel: Blueprint */}
          <div key="blueprint" className="h-full"><DashboardPanel label="Blueprint">
            {blueprint === null ? (
                      ...
                      <div key={iter.id} className="flex items-center shrink-0">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            const resp = await authFetch(`/api/projects/${id}/blueprint?iteration_id=${iter.id}`);
                            if (resp.ok) { const bp = await resp.json(); setBlueprint(bp.content || bp); }
                          }}
                        ...
                        >
                          <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                            isFinalized ? "bg-success" : isReady ? "bg-blue-400" : "bg-amber-400"
                          }`} />
                          {name}
                          {isFinalized && <span className="text-[8px] text-success/60">Finalized</span>}
                          {isReady && <span className="text-[8px] text-blue-400/60">Ready</span>}
                        </button>
                      </div>
                    );
                  })}
                ...
                {v2NoType ? (
                  <div className="py-6 text-center">
                    ...
                    <p className="text-[10px] font-body text-muted-foreground/30">
                      Start a new session and choose a type to see which sections this release will cover.
                    </p>
                    <Link
                      href={`/projects/${project.id}/sessions/new`}
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      <div className={`relative shrink-0 ${"w-12 h-12"}`}>
                        <svg className={`${"w-12 h-12"} -rotate-90`} viewBox="0 0 36 36">
                          <circle cx="18" cy="18" r="15" fill="none" stroke="currentColor" className="text-white/[0.06]" strokeWidth="3" />
                          <circle
                            cx="18" cy="18" r="15" fill="none"
                            stroke="currentColor"
                            className={filledSections === countSections.length ? "text-success" : "text-primary"}
                            strokeWidth="3"
                            strokeLinecap="round"
                            strokeDasharray={`${(filledSections / countSections.length) * 94.2} 94.2`}
                          />
                        </svg>
                        <span className={`absolute inset-0 flex items-center justify-center font-bold text-foreground/70 ${"text-xs"}`}>
                          {filledSections}
                        </span>
                      </div>
                      <div>
                ...
                {iterations.length > 0 && iterations.every((i) => i.status === "locked") && (
                  <button
                    onClick={async () => {
                      ...
                      const resp = await authFetch(`/api/projects/${project.id}/iterations`, { method: "POST" });
                      if (resp.ok) {
                        const newIter = await resp.json();
                        setIterations((prev) => [...prev, newIter]);
                        setActiveIterationId(newIter.id);
                        const bpResp = await authFetch(`/api/projects/${id}/blueprint?iteration_id=${newIter.id}`);
                        if (bpResp.ok) { const bp = await bpResp.json(); setBlueprint(bp.content || bp); }
                      }
                    }}
          ...
          {/* Panel: Diagrams */}
          <div key="diagrams" className="h-full"><DashboardPanel label="Diagrams">
            {diagrams.length === 0 ? (
              <p className="text-xs text-muted-foreground/50 font-body">No diagrams yet. Start a session and ask for a flow, architecture, or ERD diagram.</p>
            ) : (
              <div className="space-y-2">
                {diagrams.slice(0, 6).map((d) => {
                  const typeLabels: Record<string, string> = { flow: "User Flow", architecture: "Architecture", erd: "Data Model", wireframe: "Wireframes" };
                  const typeColors: Record<string, string> = { flow: "text-blue-400", architecture: "text-success", erd: "text-violet-400", wireframe: "text-amber-400" };
                  const nodeCount = d.diagram.nodes?.length ?? d.diagram.tables?.length ?? d.diagram.screens?.length ?? 0;
                  return (
                    <Link
                      key={d.session_id}
                    ...
                    >
                      <div className="min-w-0">
                        <span className={`text-[10px] font-body font-medium uppercase tracking-wide ${typeColors[d.diagram.type] ?? "text-white/50"}`}>
                ...
                {diagrams.length > 6 && (
                  <p className="text-[10px] font-body text-muted-foreground/40 px-3">+{diagrams.length - 6} more</p>
                )}
              </div>
            )}
          </DashboardPanel></div>
          ...
          {/* Panel: Board */}
          <div key="board" className="h-full"><DashboardPanel label="Board">
            {boardSummary === null ? (
              <p className="text-xs text-muted-foreground/50 font-body">Loading…</p>
            ) : boardSummary.columns?.length > 0 ? (
              <BoardPanelContent columns={boardSummary.columns} projectId={project.id} layout={boardLayout} />
            ) : (
          ...
          {/* Panel: Analytics */}
          <div key="analytics" className="h-full"><DashboardPanel label="Analytics">
            {analyticsMetrics === null ? (
              <p className="text-xs text-muted-foreground/50 font-body">Loading…</p>
            ) : analyticsMetrics.total_sessions === 0 ? (
              <p className="text-xs text-muted-foreground/50 font-body mb-3">No session data yet.</p>
            ) : (
          ...
          {/* Panel: Repository */}
          <div key="repository" className="h-full"><DashboardPanel label="Repository">
      ...
      {project && (
        <EditProjectDialog
          ...
          project={project}
          onSaved={(data) => setProject({ ...project, ...data, description: data.description ?? null })}
        />
      )}
