...
import { Sparkles, Check, X, Pencil, Rocket, Zap, Bug, Search, Layers, Wrench, Gauge, Timer, Microscope } from "lucide-react";
...
const TYPE_FOCUS_SECTIONS: Record<string, string[]> = {
  large_feature: ["project_overview", "goals_constraints", "users_personas", "team_capacity", "architecture", "tech_stack", "api_integrations", "ui_ux", "security_compliance", "infrastructure", "risks_unknowns", "out_of_scope", "open_questions"],
...
export default function NewSessionPage() {
  ...
  const [coverage, setCoverage] = useState<{ overall: number; grade: string; hasBlueprint: boolean } | null>(null);
  ...
  const [activeSessions, setActiveSessions] = useState<Array<{ id: string; title: string | null; release_name: string | null }>>([]);
    ...
    authFetch(`/api/projects/${projectId}/session-suggestions`)
      .then(async (resp) => {
        if (resp.ok) {
          ...
          if (data.first_release_complete !== undefined) setFirstReleaseComplete(data.first_release_complete);
          ...
          if (!data.has_blueprint) {
            ...
            const desc = data.project_description
              ? (data.project_description.charAt(0).toLowerCase() + data.project_description.slice(1)).replace(/\.$/, "")
              : "";
  ...
  const detectType = useCallback((text: string) => {
  ...
  async function createSession() {
    ...
    const selectedPersona = selectedIdx !== null
    ...
    if (selectedType && coverage?.hasBlueprint) {
      await authFetch(`/api/projects/${projectId}/iterations/${(await authFetch(`/api/projects/${projectId}/iterations`).then(r => r.json())).find((i: { status: string }) => i.status === "planning")?.id}`, {
        method: "PATCH",
    ...
    try {
      ...
      if (idea.trim() && !ideaContainsDesc) {
        const checkResp = await authFetch(
          `/api/projects/${projectId}/sessions/check-relevance`,
          { method: "POST", body: JSON.stringify({ initial_idea: idea }) },
        );
        if (checkResp.ok) {
  ...
  return (
    <div className="min-h-screen">
      <main className="mx-auto max-w-2xl px-6 py-14">
          ...
          <div className="mb-8 animate-slide-up stagger-2">
            <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-6 text-center">
              ...
              {activeSessions.length > 0 && (
                <button
                  onClick={() => router.push(`/projects/${projectId}/sessions/${activeSessions[0].id}`)}
            ...
            <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3">
              <div className="flex items-center justify-between">
                <p className="text-[11px] font-body text-foreground/70">
                  You have {activeSessions.length} active release{activeSessions.length > 1 ? "s" : ""}.{" "}
                  <button
                    onClick={() => router.push(`/projects/${projectId}/sessions/${activeSessions[0].id}`)}
          ...
          <div className="mb-6 animate-slide-up stagger-2">
            ...
            {coverage?.hasBlueprint && (
              <p className="text-[11px] font-body text-muted-foreground/60 mb-3">
                Blueprint is at {coverage.overall}% coverage (grade {coverage.grade}). These sessions will help fill the gaps:
              </p>
            )}
            <div className="space-y-2">
              {suggestions.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className={`text-sm font-body font-medium group-hover:text-foreground ${isSelected ? "text-foreground" : "text-foreground/90"}`}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {s.persona && (
                          <span className="text-[9px] font-body text-muted-foreground/40 px-1.5 py-0.5 rounded bg-muted/30">
                            {s.persona === "default" ? "Engineer" : s.persona === "pm" ? "PM" : s.persona === "architect" ? "Architect" : s.persona === "challenger" ? "Challenger" : "Mentor"}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {s.sections && s.sections.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {s.sections.map((sec) => (
                          <span key={sec} className={`text-[9px] font-body px-1.5 py-0.5 rounded ${isSelected ? "bg-primary/15 text-primary/70" : "bg-muted/40 text-muted-foreground/50"}`}>
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <span className="text-[10px] font-body text-muted-foreground/30">or describe your own</span>
              <div className="flex-1 h-px bg-border/30" />
            </div>
          </div>
        )}
          ...
          <div className="mb-6 animate-slide-up stagger-2">
            ...
            {parentOos && (
              <p className="text-[11px] font-body text-muted-foreground/50 mb-3">
                From previous release: {parentOos.slice(0, 120)}{parentOos.length > 120 ? "..." : ""}
              </p>
            )}
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (isSelected) {
                        ...
                        const desc = projectDesc ? (projectDesc.charAt(0).toLowerCase() + projectDesc.slice(1)).replace(/\.$/, "") : "";
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-body font-medium ${isSelected ? "text-foreground" : "text-foreground/80"}`}>
                        {t.label}
                      </span>
                      {isDetected && (
                    ...
                    {isSelected && (t.sections || []).length > 0 && (
                      <div className="mt-1.5 ml-6 flex flex-wrap items-center gap-1">
                        {t.sections_count <= 6 ? (
                          <>
                            {(t.sections || []).map((sec) => (
                              <span key={sec} className="text-[8px] font-body px-1.5 py-0.5 rounded bg-primary/15 text-primary/70">
                                {TYPE_SECTION_LABELS[sec] || sec.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}
                              </span>
                            ))}
                            {t.sections_count < 13 && (
        ...
        {(firstReleaseComplete || !coverage?.hasBlueprint) && <div className="space-y-5 animate-slide-up stagger-2">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-xs font-body font-medium text-muted-foreground">
                {coverage?.hasBlueprint ? "What should this session focus on?" : "What are you planning?"}
              </Label>
              <button
                type="button"
            ...
            <Textarea
              ref={ideaRef}
              value={idea}
              onChange={(e) => { setIdea(e.target.value); if (selectedIdx !== null) setSelectedIdx(null); detectType(e.target.value); }}
              placeholder={coverage?.hasBlueprint
          ...
          {suggestion && (
            <div className="rounded-lg border border-primary/30 bg-primary/5 overflow-hidden animate-slide-up">
              <div className="px-4 py-2.5 border-b border-primary/20 flex items-center gap-2">
                ...
                <span className="text-xs font-body font-medium text-primary">Suggested rewrite</span>
              </div>
