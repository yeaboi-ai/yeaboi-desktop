...
// Roadmap intake — point yeaboi at the quarterly roadmap, pick an item, and
// it becomes a plan: the engine resolves the item into the description a
// plan opens with, and the room takes it from there.
...
import { createChat, sessionIdOf } from '@/lib/yeaboi/chat';
import {
  type ModeRunState,
...
function RoadmapBody() {
  ...
  async function makePlan(index: number, name: string) {
    if (creating >= 0) return;
    setCreating(index);
    setError('');
    try {
      // The engine resolves the roadmap item into the description a plan
      // opens with; the room sends it as the first turn.
      ...
      router.push(`/planning/${encodeURIComponent(sessionIdOf(view))}`);
    } catch (e) {
      setError((e as Error).message);
      setCreating(-1);
    }
    ...
    <div className="space-y-4">
      <header className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-1">
            Point yeaboi at the quarterly roadmap and it proposes what to plan next. Each pick opens
            as a plan.
          </p>
        </div>
        <Link href="/planning" className="text-[12px] text-muted-foreground hover:text-foreground">
          Back
        </Link>
      </header>
      ...
      <Section title="Where does the roadmap live?">
        ...
        <label className="block">
          <span className="text-[12px] font-body text-muted-foreground">{chosen.prompt}</span>
          <input
            type="text"
            value={locator}
              ...
              <Section key={`${project.name}-${index}`} title={project.name}>
                ...
                <div className="mt-3">
                  <Button
                    size="sm"
                    disabled={creating >= 0}
                    onClick={() => void makePlan(index, project.name)}
                  >
                    {creating === index ? 'Opening…' : 'Plan this'}
                  </Button>
                </div>
              </Section>
            ))}
...
export default function PlanningFromRoadmapPage() {
  return (
