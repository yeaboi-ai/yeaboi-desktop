...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    examples: ['GET /users/42 to read a user, POST /users to create one.'],
    learnMore: [
  ...
  },
  {
    ...
    contextHint:
      'Often discussed as an alternative to REST when the frontend needs flexible data shapes.',
    examples: [
      "One request returns 'a user with their last 3 orders and total spend' — no extra round trips.",
    ],
    learnMore: [{ label: 'graphql.org', url: 'https://graphql.org/' }],
  },
  {
    slug: 'auth',
    term: 'Auth',
    ...
    contextHint:
      "When the AI asks about 'auth', it means how users prove their identity to your app.",
    examples: ['Logging in with email + password.', 'Signing in with Google.'],
    learnMore: [
      {
        label: 'Auth0: What is authentication?',
  ...
  },
  {
    ...
    examples: ["An 'admin' can delete posts; a 'member' can only read them."],
    learnMore: [
      {
  ...
  },
  {
    ...
    learnMore: [{ label: 'oauth.net', url: 'https://oauth.net/2/' }],
  },
  {
    slug: 'jwt',
    term: 'JWT',
    ...
    contextHint:
      "Often used in stateless APIs so the server doesn't have to look up sessions in a database.",
    examples: ['After logging in, your app stores a JWT and sends it with every API call.'],
    learnMore: [{ label: 'jwt.io intro', url: 'https://jwt.io/introduction' }],
  },
  {
    slug: 'webhook',
    term: 'Webhook',
    ...
    examples: ['Stripe calls your webhook when a payment succeeds so you can mark the order paid.'],
    learnMore: [{ label: 'What is a webhook?', url: 'https://en.wikipedia.org/wiki/Webhook' }],
  },
  {
    slug: 'schema',
    term: 'Schema',
    ...
    contextHint:
      'Usually refers to a database schema (table layout) or an API schema (request/response shape).',
    examples: ["A 'users' table schema: id (number), email (text), created_at (timestamp)."],
    learnMore: [
  ...
  },
  {
    ...
    examples: ["Adding a 'phone_number' column to the users table needs a migration."],
    learnMore: [
      {
        label: 'Wikipedia: Schema migration',
  ...
  },
  {
    ...
    contextHint:
      "When the AI says 'put it on a queue', it means run it later, not while the user waits.",
    examples: ['Sending a welcome email goes on a queue so signup feels instant.'],
    learnMore: [{ label: 'Message queues', url: 'https://en.wikipedia.org/wiki/Message_queue' }],
  },
  {
    slug: 'cache',
    term: 'Cache',
    ...
    examples: ["Your browser caches images so reload doesn't redownload them."],
    learnMore: [
      {
  ...
  },
  {
    ...
    examples: ['Cloudflare and Fastly are CDNs many sites use.'],
    learnMore: [
      {
  ...
  },
  {
    ...
    examples: ['Next.js pages are SSR by default.'],
    learnMore: [{ label: 'Vercel: SSR explained', url: 'https://vercel.com/docs/rendering' }],
  },
  {
    slug: 'microservice',
    term: 'Microservice',
    ...
    examples: ["Split a monolith into 'auth service', 'billing service', 'notifications service'."],
    learnMore: [
      {
        label: 'Martin Fowler on microservices',
  ...
  },
  {
    ...
    examples: ['Most early-stage apps are monoliths until they hit scale problems.'],
    learnMore: [
  ...
  },
  {
    ...
    examples: ["GitHub Actions running 'npm test' on every push."],
    learnMore: [
  ...
  },
  {
    ...
    examples: ['STRIPE_API_KEY=sk_test_... in development, sk_live_... in production.'],
    learnMore: [{ label: '12-factor config', url: 'https://12factor.net/config' }],
  },
  {
    slug: 'rate-limit',
    term: 'Rate limit',
    ...
    examples: ['Stripe lets you make 100 requests per second per account.'],
    learnMore: [
      {
  ...
  },
  {
    ...
    examples: ["Airbnb's MVP was a single rented air mattress in a founder's apartment."],
    learnMore: [
  ...
  },
  {
    ...
    aliases: ['regressions'],
    definition:
      'A bug that re-appears in code that used to work — usually caused by an unrelated change.',
    contextHint: 'What you guard against by writing tests before changing something risky.',
    examples: ["Login broke after a refactor that 'shouldn't have touched auth'."],
    learnMore: [
      {
        label: 'Wikipedia: Software regression',
  ...
  },
  {
    ...
    examples: ['Sentry for errors, Datadog for metrics, OpenTelemetry for traces.'],
    learnMore: [
      {
        label: 'Honeycomb: Observability 101',
  ...
  },
  {
    ...
    contextHint:
      "The slowest tests, but the only kind that catches 'all the parts wired together'.",
    examples: ["Playwright clicks 'Sign up', fills the form, and verifies the dashboard loads."],
    learnMore: [{ label: 'Playwright docs', url: 'https://playwright.dev/' }],
  },
  {
    slug: 'mock',
    term: 'Mock',
    ...
    examples: ["Mock the 'send email' function so tests don't actually send mail."],
    learnMore: [
      {
        label: 'Martin Fowler on mocks',
  ...
  },
  {
    ...
    examples: ['git push main → CI runs tests → Vercel deploys the new build.'],
    learnMore: [
