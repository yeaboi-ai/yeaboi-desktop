...
// one surface says them: the health banner and the Settings → System row.
//
...
export function providerFailureDetail(
  ...
  switch (code) {
    case 'PROVIDER_INVALID_KEY':
      return `${name} rejected ${yours}API key. Check it in Settings → System — an unreplaced placeholder and a revoked key both look like this.`;
    case 'PROVIDER_CREDIT_EXHAUSTED':
      return `${name} reports no credit left on ${yours}account. Top it up, or switch provider in Settings → System.`;
    case 'PROVIDER_RATE_LIMITED':
      return `${name} is rate-limiting requests. This usually clears on its own in a minute or two.`;
    default:
      return `${name} is not responding. This usually clears on its own; if it persists, check Settings → System.`;
  }
}
