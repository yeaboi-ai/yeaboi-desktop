#!/usr/bin/env node
// Stage the Pythons the packaged app runs — two python-build-standalone
// runtimes under resources/, shipped by electron-builder as extraResources:
//
//   resources/py           the yeaboi TUI backend (a released PyPI wheel);
//                          sidecar.ts spawns `Resources/py/bin/python3 -m
//                          yeaboi app`
//   resources/py-planning  the planning-platform backend (a wheel built by
//                          CI from the sibling repo — it has no PyPI
//                          release); planning.ts spawns `... -m uvicorn
//                          app.main:create_app`
//
...
const STAGE_PLANNING = join(ROOT, 'resources', 'py-planning');
...
async function stagePlanning({ platform, arch, planningWheel }) {
  ...
  rmSync(STAGE_PLANNING, { recursive: true, force: true });
  mkdirSync(STAGE_PLANNING, { recursive: true });
  run('tar', ['-xzf', archive, '-C', STAGE_PLANNING, '--strip-components', '1']);

  const python = pythonPath(STAGE_PLANNING, platform);
  if (!existsSync(python)) throw new Error(`the runtime unpacked without ${python}`);

  console.log(`installing planning backend from ${planningWheel}`);
  run('uv', ['pip', 'install', '--python', python, planningWheel]);

  const removed = await prune(STAGE_PLANNING);
  console.log(`pruned ${removed} cache/test directories`);
  // Keep the wheel itself beside the runtime: the in-app voice pack installs
  // `<wheel>[voice]` from it (src/main/voice-pack.ts) — the backend has no
  // package index to install from.
  const wheelName = planningWheel.split('/').pop();
  writeFileSync(join(STAGE_PLANNING, wheelName), readFileSync(planningWheel));
  // The two commands the sidecars actually run, proven before signing.
  run(python, ['-c', 'import app.main; print("staged planning backend")']);
  run(python, ['-c', 'import uvicorn']);
  writeFileSync(
    join(STAGE_PLANNING, 'planning-bundle.json'),
    `${JSON.stringify({ target, python: PBS_PYTHON, pbs: PBS_TAG, wheel: planningWheel }, null, 2)}\n`,
  );
}
