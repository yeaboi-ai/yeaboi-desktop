...
OUTFIT_HEADROOM = 40
#: Pixels per art cell on each canvas. The art is 64 cells across, whatever
#: resolution it is drawn at, so the cell follows the art rather than a number.
CELLS_ACROSS = 64
CELL = SOURCE_SIZE[0] / CELLS_ACROSS
#: The desktop pet draws the same art larger; everything above scales with it.
PET_WIDTH = 480
PET_CELL = PET_WIDTH / CELLS_ACROSS
PET_SCALE = PET_WIDTH / SOURCE_SIZE[0]
PET_HEADROOM = round(OUTFIT_HEADROOM * PET_SCALE)
PET_SIZE = (PET_WIDTH, round(SOURCE_SIZE[1] * PET_SCALE) + PET_HEADROOM)
