...
`provider-icon.tsx` keeps two tiers underneath every mark — per-vendor lettering
(`MONOGRAMS`) and a per-family glyph (`FAMILY_GLYPHS`) — so removing any mark
above is a one-line change that leaves the surface rendering correctly. A vendor
with no shippable mark uses those tiers rather than a reconstruction: a logo
redrawn from memory would be a wrong mark rather than an absent one.
